...
def get_related_activities(domain, goal, goal_category, kb, n=3):
    ...
    # ✅ collect topics from KB (FIXED STRUCTURE)
    for lvl in kb.get(domain, {}):
        for t in kb[domain][lvl]:   # ✅ directly iterate list

            items.append(t.get("topic", ""))
    ...
    try:
        ...
        # 🔥 STRICT FILTERING (ACCURACY BOOST)
        THRESHOLD = 0.25

        filtered = [i for i in range(len(sims)) if sims[i] > THRESHOLD]

        # fallback if nothing passes
        if not filtered:
            filtered = sims.argsort()[::-1][:10]

        # sort filtered by similarity
        filtered = sorted(filtered, key=lambda i: sims[i], reverse=True)
